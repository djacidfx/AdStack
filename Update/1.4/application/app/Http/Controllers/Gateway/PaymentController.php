...
class PaymentController extends Controller
{
    ...
    public function depositInsert(Request $request)
    {
        if($request->gateway == 'balance'){
            ...
            $subscription->advertiser_id = $user->id;
            $subscription->plan_id = $plan->id;
            $subscription->amount = $plan->price;
            $subscription->type = $plan->type;
            $subscription->impression_point = ($plan->type == 'impression') ? $plan->points : $subscription->impression_point;
            $subscription->click_point = ($plan->type !== 'impression') ? $plan->points : $subscription->click_point;
            $subscription->save();

            $user->balance -= $plan->price;

            if($plan->type == 'impression'){
                $user->impression += $plan->points;
            }else{
                $user->click += $plan->points;
            }
            $user->save();
