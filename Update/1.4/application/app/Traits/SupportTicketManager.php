...
trait SupportTicketManager
{
    ...
    public function supportTicket()
    {
        ...
        if(auth()->guard('publisher')->check()){
            $supports = SupportTicket::where('publisher_id', $user->id)->orderBy('id', 'desc')->paginate(getPaginate());
        }else{
            $supports = SupportTicket::where($this->column, $user->id)->orderBy('id', 'desc')->paginate(getPaginate());
        }

        return view($this->activeTemplate . $this->userType . '.support.index', compact('supports', 'pageTitle','layout'));
    }
    ...
    public function openSupportTicket()
    {
        ...
        $layout    = $this->layout;
        return view($this->activeTemplate . $this->userType . '.support.create', compact('pageTitle', 'user','layout'));
    }
    ...
    public function storeSupportTicket(Request $request)
    {
        ...
        if($request->user_type =="publisher"){
    ...
    public function viewTicket($ticket)
    {
        ...
        if(auth()->guard('publisher')->check()){
            $myTicket = SupportTicket::where('ticket', $ticket)->where('publisher_id', $userId)->orderBy('id', 'desc')->firstOrFail();
        }else{
    ...
    public function replyTicket(Request $request, $id)
    {
        ...
        if ($this->userType == 'admin') {
            ...
            if ($ticket->user_id != 0) {
                $createLog = true;
                $user = $ticket->advertiser ?? $ticket->publisher;
            }
