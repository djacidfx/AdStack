...
class SiteController extends Controller
{
    ...
    public function contactSubmit(Request $request)
    {
        ...
        $ticket->user_id = auth()->guard('publisher')->check()
        ? auth()->guard('publisher')->user()->id
        ...
        $adminNotification->user_id = auth()->guard('publisher')->check()
        ? auth()->guard('publisher')->user()->id
        : (auth()->guard('advertiser')->check()
            ? auth()->guard('advertiser')->user()->id
            : 0);
    ...
    // advertisr login
    public function showadvertiserLoginForm()
    {
        $pageTitle = "Login";
        return view($this->activeTemplate . 'advertiser.login', compact('pageTitle'));
    }
    ...
    public function showRegistrationForm()
    {
        ...
        return view($this->activeTemplate . 'register', compact('pageTitle','mobileCode','countries'));
    }

    // advertiser
    public function showadvertiserRegistrationForm()
    {
        $pageTitle = "Register";
        $info = json_decode(json_encode(getIpInfo()), true);
        $mobileCode = @implode(',', $info['code']);
        $countries = json_decode(file_get_contents(resource_path('views/includes/country.json')));
        return view($this->activeTemplate . 'advertiser.register', compact('pageTitle','mobileCode','countries'));
    }
