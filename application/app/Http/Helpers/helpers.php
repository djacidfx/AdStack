...
{
    if ($type) {
        return 'advertiser.deposit.history';
    } else {
        return 'user.deposit';
    }
}
...
function isSubscribe($userId){
    ...
    $subscription = Purchase::where('advertiser_id',$userId)->orderBy('id', 'desc')->get();
...
function getSubscriptionVisitor($userId, $subscriptionType) {
    $subscription = Purchase::where('advertiser_id', $userId)
